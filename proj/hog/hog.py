...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # Remember to call NUM_ROLLS times
    res, flag = 0, False
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # Careful, the score could be very large so that number // 10 wouldn't exactly be the tens
...
def play(strategy0, strategy1, update,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    goal0, goal1 = score0, score1
    # Careful, should pay more attention to the parameter list
    while goal0 < goal and goal1 < goal:
        if who == 0:
            num_roll0 = strategy0(goal0, goal1)
            goal0 = update(num_roll0, goal0, goal1, dice)
        else:
            num_roll1 = strategy1(goal1, goal0)  # Careful here, order matters
            goal1 = update(num_roll1, goal1, goal0, dice)
        who = 1 - who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(100):
        for j in range(100):
            ...
            if num_roll_flag == -1:
                num_roll_flag = num_roll
            else:
                if num_roll_flag != num_roll:
                    return False
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # A little bit complicated, it means we call roll_dice TOTAL_SAMPLES times, and average
    # the result from calling roll_dice(*args)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    # roll_dice(num_rolls, ...) with different NUM_ROLLS have different results, and we are
    # averaging these results.
    averaged_dice, maximum_roll, maximum_average = make_averaged(roll_dice,total_samples) , 0, 0
    for num_rolls in range(1,11):
        averaged_res = averaged_dice(num_rolls, dice)
        if averaged_res > maximum_average:
            maximum_roll, maximum_average = num_rolls, averaged_res
    return maximum_roll
    # END PROBLEM 9
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    Pig Tail Rule: A player who chooses to roll zero dice scores 2 * abs(tens - ones) + 1 points;
    where tens, ones are the tens and ones digits of the opponent's score.
    The ones digit refers to the rightmost digit and the tens digit
    refers to the second-rightmost digit.
    """
    # BEGIN PROBLEM 10
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score > opponent_score:
        for num_rolls in range(3):
            new_score = square_update(num_rolls, score, opponent_score, six_sided)
            if new_score > GOAL:
                return num_rolls
    else:
        total_samples = 500
        averaged_dice = make_averaged(roll_dice, total_samples)
        if square_update(0, score, opponent_score,six_sided) > averaged_dice(6, six_sided):
            return 0
        else:
            return 6
