...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        p_place = self.place
        curr_tile = 0
        while not p_place.is_hive:
            if len(p_place.bees) > 0 and self.lower_bound <= curr_tile <= self.upper_bound:
                return random_bee(p_place.bees)
            p_place, curr_tile = p_place.entrance , curr_tile + 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        # 先计算要扣除多少血量
        copy_list, total_damage = self.place.bees[:], amount
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            bees_available = self.place.bees
            if len(bees_available) > 0:
                bee_to_chew = random_bee(bees_available)
                bee_to_chew.reduce_health(bee_to_chew.health)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        # Careful not to recursively call self.add_insect()
